              ...
              <tr>
                {fields.map((f, k) =>
                  typeof f === "string" ? (
                    ...
                    <th key={k} style={{ textAlign: f.align || "left" }}>
                      {f.label}
                    </th>
                  )
                )}
            ...
            {data.length === 0 ? (
              ...
              data.map((i, k) => (
                <tr key={k}>
                  {fields.map((f, m) => {
                    if (typeof f === "string")
                      return <td key={m}>{i[f] || ""}</td>;
                    return (
                      <td key={m} style={{ textAlign: f.align || "left" }}>
                        {f.render ? f.render(i[f.key], i, k) : i[f.key]}
                      </td>
                    );
                  })}
                </tr>
        ...
        <div className="tbl-footer">
          ...
          <div>
            <Pager
              page={status.page}
              total={total}
              onChange={(page) => onPage(page, status.limit)}
            />
          </div>
        </div>
      )}
